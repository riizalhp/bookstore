...
function insertDataBuku() {
    dataBuku.forEach((buku) => {
        const query = `INSERT INTO buku (id_buku, kategori, nama_buku, tahun, harga, stok, penerbit) VALUES (?, ?, ?, ?, ?, ?, ?)`;
        const values = [buku.id_buku, buku.kategori, buku.nama_buku, buku.tahun, buku.harga, buku.stok, buku.penerbit];
...
function insertDataPenerbit() {
    dataPenerbit.forEach((penerbit) => {
        const query = `INSERT INTO penerbit (id_penerbit, nama, alamat, kota, telepon, email) VALUES (?, ?, ?, ?, ?, ?)`;
        const values = [penerbit.id_penerbit, penerbit.nama, penerbit.alamat, penerbit.kota, penerbit.telepon, penerbit.email];
